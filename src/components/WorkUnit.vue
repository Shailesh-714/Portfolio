<template>
    <div :class="['flex flex-col lg:grid lg:grid-cols-2 lg:gap-x-6 transition-opacity motion-reduce:transition-none duration-500', showProject ? 'opacity-1' : 'opacity-0']">
        <div v-if="!project.alignLeft" class="order-first pb-4 lg:pb-0 lg:order-none flex items-center">
            <img
        ...
        <div class="flex flex-col space-y-4" :class="project.alignLeft ? 'text-left' : 'lg:text-right'">
            ...
            <p class="text-justify text-slate-500 dark:text-slate-300">{{ project.description }}</p>
